...
//            "SELECT a.*, pi.* FROM agreement a " +
//            "LEFT JOIN (" +
//            "SELECT DISTINCT ON (agreement_external_id) * FROM payment_instrument ORDER BY agreement_external_id, created_date DESC" +
//            ") pi ON pi.agreement_external_id = a.external_id " +
            "WHERE a.external_id = :externalId";

    // @TODO(sfount): reasonably valid question about whether this should be projected flat or if it should be joined
    // joining allows future things like stored cards to be presented at the cost of performance
    ...
    public AgreementDao(Jdbi jdbi) {
        this.jdbi = jdbi;
//        jdbi.registerRowMapper(AgreementEntity.class, new AgreementMapper());
    }
